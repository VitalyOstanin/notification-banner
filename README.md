...
- The settings `changed` handler reapplies the position and re-runs the
  ...
  While the preferences window is open it also shows a persistent sample banner at
  the configured placement (driven by two internal keys the window sets over
  GSettings), so the position is visible the whole time you adjust settings; the
  sample clears when the window closes.
